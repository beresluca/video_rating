"""
CommGame experiment processing script for subsequent video rater task.
...
USAGE: python3 combineVideos.py INPUT_DIR PAIR_NO SESSION
...
- Important timestamps are saved out to an npz (numpy) file at:
    [INPUT_DIR]/pair[PAIR_NO]_[SESSION]_combined_video_start.npz
  The file contains UNIX timestamps:
...
- Videos are only combined from the "n"th frame on, because frame capture timestamps are variable (jitter) for the
  first few frames, probably due to an initial period needed for stable frame rate.
  HARDCODED! "n" is 10 at the moment.
...
def extract_video_times_mat(input_dir, pair_no, session):
    ...
    :return: start_time: Float, timestamp of task (recording) start
    :return: stop_time: Float, timestamp of task (recording) end
    :return: frame_times: Numpy array of frame capture timestamps
...
def combine_frames(input_dir, pair_no, session='freeConv', start_frame=10):
    """
    ...
    :param input_dir:   Path to folder containing the video and timestamp files for given pair and session.
                        Gets globbed recursively for relevant files.
    ...
    # The method below takes too much time, only use it for trouble-shooting, if there is stg fishy about frame numbers
    # count the frames of the videos with slow but accurate method
    # mordor_frame_count = count_frames_manual(video_mordor)
    # gondor_frame_count = count_frames_manual(video_gondor)
    ...
    if video_m_h != expected_h or video_m_w != expected_w:
        raise ValueError('Unexpected frame size! Should be 1920 x 1080 for both videos!')
